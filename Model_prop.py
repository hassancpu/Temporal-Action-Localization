...
from data_loader_prop import Dataloader
...
drop_2 = Dropout(0.1)
d_3 = Dense(units_out, activation = 'sigmoid')
...
def smooth_l1(y_true, y_pred, delta= 0.1):
    
    
    diff = tf.abs(y_true - y_pred)
    
    low = tf.cast(diff <= delta, dtype= tf.float32)
    high = tf.cast(diff > delta, dtype= tf.float32)
    
    low_loss = 0.5*tf.square(diff)
    low_loss = tf.multiply(low_loss, low)
    
    high_loss = delta*(diff - 0.5*delta)
    high_loss = tf.multiply(high_loss, high)
...
loss = smooth_l1(score_true_ref, score_ref, 0.1)
...
class CustomSaver(Callback):
  def on_epoch_end(self, epoch, accuracy, logs= {}):
    # save the trained model after each epoch
    if epoch +1 > 10:
        model.save("./Trained_Model/Proposal Score Prediction/First Model"
                   "/model {}, v_loss={:9.6f}, loss={:9.6f}".format(epoch+1, accuracy['val_loss']
                   , accuracy['loss']))
...
if __name__=='__main__': 
    ...
    train_loader= Dataloader(256, 'train')
    val_loader= Dataloader(256, 'val')   
