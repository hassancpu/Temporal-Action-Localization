...
def get_vote_and_ave(score_matrix):
    ...
    return list(vote_arr), list(ave_arr), score_matrix_ref
...
if __name__=='__main__':
   ...
   for vid_idx in range(dataloader.batch_num):
        ...
        vote_V = 1
        start_idx_part = np.where(np.array(start_vote) >=vote_V)[0]
        end_idx_part = np.where(np.array(end_vote) >=vote_V)[0]
        ...
        tmp = [i*16 + 8 for i in range(0,len(start_ave))]
        
        f_start = interp1d(tmp, start_ave, kind= 1, fill_value= 'extrapolate')
        f_end = interp1d(tmp, end_ave, kind= 1, fill_value= 'extrapolate')
        f_action = interp1d(tmp, action_ave, kind= 1, fill_value= 'extrapolate')
            ...
            for j in range(len(end_idx)):
                ...
                if candi_end > candi_start:
                    ...
                    if cnt <= 20:
                        ...
                        n_start = np.argmax(start_all[candi_start*16:candi_start*16 + 16]) + 1
                        n_end = np.argmax(end_all[candi_end*16:candi_end*16 + 16]) + 1
                        print(n_start)
                        print(n_end)
                        start_time = (candi_start * 16 + n_start) /cur_fps
                        end_time = (candi_end * 16 + n_end) /cur_fps
